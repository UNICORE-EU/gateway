...
public class POSTHandler
{
	...
	public void invoke(final RawMessageExchange exchange) throws Exception
	{
		...
		try
		{
			...
			if(site == null){
				...
				throw new FileNotFoundException( "Could not resolve a site for <"+getAddress(exchange)+">");
			}
			...
			if (chunked)
			{
				requestentity = new AbstractStreamReaderRequestEntity(chunked)
				{
					...
					{
						return new ReaderInputStream(exchange.getReader(), "UTF-8"); 
					}
			...
			throws Exception
	{
		...
		if(!exchange.isSOAP()){
			...
			if (certPath != null)
			{
				post.addHeader(consignorProducer.getConsignorHeader(certPath,clientIP));
			}
		}
			...
			RawMessageExchange exchange, Logger log) throws Exception
	{
		...
		while (r > 0)
		{
			target.write(new String(buf, 0, r, responseCharSet));
	...
	public static void writeToOutputStream(RawMessageExchange ex, OutputStream os, X509Certificate[] certs, 
			String ip, IConsignorProducer producer, Logger log) throws Exception
	{
	...
	private static void writeConsignorAssertionToBaos(ByteArrayOutputStream baos, boolean createHeader, 
			...
			IConsignorProducer consignorProducer, Logger log) throws Exception
	{
